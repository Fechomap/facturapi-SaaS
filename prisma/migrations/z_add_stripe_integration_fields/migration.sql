-- AlterTable: Add columns only if the tables and columns don't exist
DO $$ 
BEGIN 
    -- First check if the subscription_plans table exists
...
-- Handle potential duplicate values before creating unique index only if the table exists
DO $$
BEGIN
    -- Check if the tenants table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tenants') THEN
...
-- CreateIndex: Only create if the table and column exist and the index doesn't exist
DO $$ 
BEGIN 
    -- Check if the tenants table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tenants') THEN
