...
${this.appliedIndexes.map((idx) => `DROP INDEX CONCURRENTLY IF EXISTS ${idx};`).join('\n')}
...
`;
    ...
    const rollbackPath = join(
      __dirname,
      `../../backups/rollback_indexes_${new Date().toISOString().substring(0, 19).replace(/[:.]/g, '-')}.sql`
    );
      ...
      const indexes = [
        {
          ...
          statement:
            'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_subscriptions_tenant_created ON tenant_subscriptions(tenant_id, created_at DESC)',
        },
        {
          name: 'idx_tenant_invoices_tenant_folio',
          statement:
            'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_invoices_tenant_folio ON tenant_invoices(tenant_id, folio_number)',
        },
        {
          name: 'idx_tenant_invoices_tenant_status',
          statement:
            'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_invoices_tenant_status ON tenant_invoices(tenant_id, status)',
        },
        {
          name: 'idx_tenant_customers_legal_name_search',
          statement:
            'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_customers_legal_name_search ON tenant_customers(tenant_id, legal_name varchar_pattern_ops)',
        },
        {
          name: 'idx_tenant_invoices_tenant_date',
          statement:
            'CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tenant_invoices_tenant_date ON tenant_invoices(tenant_id, created_at DESC)',
        },
      ];
