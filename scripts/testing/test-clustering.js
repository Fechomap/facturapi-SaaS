...
async function runClusterTest() {
  try {
    ...
    const avgResponseTime =
      results.reduce((sum, r) => sum + (r.avgResponseTime || 0), 0) / results.length;
    const totalTestTime = testEndTime - testStartTime;

    console.log(`👥 Usuarios concurrentes: ${TEST_WORKERS}`);
    console.log(`📈 Total requests: ${totalRequests}`);
    console.log(
      `✅ Requests exitosos: ${totalSuccessful} (${((totalSuccessful / totalRequests) * 100).toFixed(1)}%)`
    );
    console.log(
      `❌ Requests fallidos: ${totalFailed} (${((totalFailed / totalRequests) * 100).toFixed(1)}%)`
    );
    ...
    console.log(
      `🚀 Throughput: ${(totalRequests / (totalTestTime / 1000)).toFixed(2)} requests/segundo`
    );
    ...
    Object.entries(workerDistribution).forEach(([worker, count]) => {
      console.log(
        `   Worker ${worker}: ${count} requests (${((count / totalSuccessful) * 100).toFixed(1)}%)`
      );
    });
