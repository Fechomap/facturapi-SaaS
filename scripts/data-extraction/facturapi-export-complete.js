  ...
  try {
    while (hasMore && page <= CONFIG.MAX_PAGES_PER_TENANT) {
      try {
        ...
        const response = await axios.get('https://www.facturapi.io/v2/invoices', {
          headers: {
        ...
        log(
          'DEBUG',
          `   ✅ Página ${page}: ${invoices.length} facturas (Total: ${allInvoices.length})`
        );
        ...
        consecutiveErrors++;
        log('ERROR', `   Error en página ${page} (intento ${consecutiveErrors})`, {
        ...
        if (consecutiveErrors >= 3) {
          log(
            'ERROR',
            `   Demasiados errores consecutivos, terminando extracción para ${tenant.businessName}`
          );
          break;
        }
    ...
    log(
      'SUCCESS',
      `📡 FacturAPI completado: ${allInvoices.length} facturas en ${page - 1} páginas`
    );
    ...
    log('ERROR', `Error general extrayendo de FacturAPI: ${tenant.businessName}`, {
      error: error.message,
    });
...
function showExecutiveSummary(allData) {
  ...
  const minDate =
    dateRanges.length > 0 ? Math.min(...dateRanges.map((d) => new Date(d).getTime())) : null;
  const maxDate =
    dateRanges.length > 0 ? Math.max(...dateRanges.map((d) => new Date(d).getTime())) : null;
  ...
  console.log(`\n📁 ARCHIVOS GENERADOS:`);
  console.log(
    `   📄 CSV completo: ${CONFIG.OUTPUT_DIR}/${CONFIG.TIMESTAMP}_facturapi_complete.csv`
  );
