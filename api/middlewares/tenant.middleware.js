  ...
  try {
    ...
    if (!tenant.isActive) {
      middlewareLogger.warn(
        { tenantId },
        'Acceso denegado: Tenant marcado como inactivo administrativamente.'
      );
...
async function requireTenant(req, res, next) {
  try {
    ...
    if (subscription) {
      ...
      switch (subscription.status) {
        case 'trial':
          if (subscription.trialEndsAt && subscription.trialEndsAt > now) {
            ...
            middlewareLogger.warn(
              ...
              'Acceso denegado: Período de prueba expirado.'
            );
          }
          break;
        case 'active':
          if (subscription.currentPeriodEndsAt && subscription.currentPeriodEndsAt > now) {
            ...
            middlewareLogger.warn(
              { tenantId, currentPeriodEndsAt: subscription.currentPeriodEndsAt?.toISOString() },
              'Período activo terminado, verificando gracia.'
            );
            ...
            if (gracePeriodEnd > now) {
              ...
              middlewareLogger.info(
                ...
                'Acceso permitido: Dentro del período de gracia.'
              );
            } else {
              middlewareLogger.warn(
                { tenantId, gracePeriodEnd: gracePeriodEnd.toISOString() },
                'Acceso denegado: Período de gracia expirado.'
              );
        ...
        case 'payment_pending':
          ...
          // Re-verificar gracia por si acaso el estado cambió directamente a pending.
          {
            const referenceDateForGrace =
              subscription.currentPeriodEndsAt || subscription.trialEndsAt; // Use end date or trial end date
            if (referenceDateForGrace) {
              ...
              if (gracePeriodEnd > now) {
                ...
                middlewareLogger.info(
                  { tenantId, gracePeriodEnd: gracePeriodEnd.toISOString() },
                  'Acceso permitido: Dentro del período de gracia (estado pendiente).'
                );
              } else {
                middlewareLogger.warn(
                  { tenantId, gracePeriodEnd: gracePeriodEnd.toISOString() },
                  'Acceso denegado: Período de gracia expirado (estado pendiente).'
                );
              ...
              middlewareLogger.warn(
                { tenantId },
                'Acceso denegado: Pago pendiente sin fecha de referencia para gracia.'
              );
        ...
        default:
          ...
          middlewareLogger.warn(
            ...
            `Acceso denegado: Estado de suscripción ${subscription.status}.`
          );
          break;
      }
    } else {
      ...
      middlewareLogger.warn(
        { tenantId },
        'Acceso denegado: No se encontró suscripción para el tenant.'
      );
    }
    ...
    req.getApiKey = async () => {
      try {
        ...
        middlewareLogger.error(
          { tenantId, error: apiKeyError.message },
          'Error al obtener API Key en middleware'
        );
    ...
    middlewareLogger.error(
      { error: error.message, stack: error.stack },
      `Error en middleware requireTenant`
    );
