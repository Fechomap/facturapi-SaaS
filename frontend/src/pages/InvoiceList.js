...
const InvoiceList = () => {
  ...
  useEffect(() => {
    ...
    if (userInfoString) {
      try {
        ...
        const status =
          userInfo?.tenant?.subscriptionStatus || userInfo?.subscription?.status || 'active'; // Default to active if not found
        ...
        setSubscriptionInfo({
          ...
          paymentLink:
            userInfo?.tenant?.paymentLink ||
            'https://mock-stripe-payment-link.com/pricemockdefault/1745906401125', // Example path or default
        });
      } catch (e) {
  ...
  const handleNewInvoiceClick = () => {
    if (isSubscriptionActive) {
      ...
      const statusText =
        subscriptionInfo?.status === 'pending_payment' ? 'Pago pendiente' : 'Vencida';
      const paymentLink =
        subscriptionInfo?.paymentLink ||
        'https://mock-stripe-payment-link.com/pricemockdefault/1745906401125'; // Fallback link
    ...
    <div>
      ...
      {!isSubscriptionActive && subscriptionInfo && (
        <div className="subscription-status-banner error">
          {' '}
          {/* Use 'error' or a specific class for styling */}
          🚨 **Suscripción Inactiva:** Tu plan '{subscriptionInfo.planName}' está{' '}
          {subscriptionInfo.status === 'pending_payment' ? 'pendiente de pago' : 'vencido'}. Para
          reactivar tu servicio, por favor realiza tu pago{' '}
          <a href={subscriptionInfo.paymentLink} target="_blank" rel="noopener noreferrer">
            aquí
          </a>
          .
        </div>
              ...
              <button
                ...
                title={
                  !isSubscriptionActive
                    ? 'Suscripción inactiva - Renueva para generar facturas'
                    ...
                    {invoices.map((invoice) => (
                      <tr key={invoice.id}>
                        ...
                        <td className="actions-cell">
                          <Link to={`/facturas/${invoice.id}`} className="view-btn">
                            Ver
                          </Link>
                          <button
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (
                                  ...
                                  const motive = prompt(
                                    'Ingrese el motivo de cancelación (01, 02, 03 o 04):'
                                  );
