...
if (!prisma) {
  invoiceHandlerLogger.fatal(
    'ERROR CRÍTICO: No se pudo inicializar Prisma, ambas fuentes fallaron'
  );
}
...
export function registerInvoiceHandler(bot) {
  ...
  bot.action(/^confirmar_(?!cancelacion_)(.+)/, async (ctx) => {
    ...
    try {
      ...
      if (!ctx.userState) {
        return ctx.reply(
          'La sesión ha expirado. Por favor, comience de nuevo.',
          Markup.inlineKeyboard([[Markup.button.callback('Volver al Menú', 'menu_principal')]])
        );
      }
      ...
      if (!ctx.hasTenant()) {
        return ctx.reply(
          ...
          Markup.inlineKeyboard([[Markup.button.callback('📝 Registrarme', 'start_registration')]])
        );
      }
      ...
      if (!canGenerateResult.canGenerate) {
        ...
        if (
          ...
          const paymentLink =
            ...
            'https://mock-stripe-payment-link.com/pricemockdefault/1745906401125'; // Fallback
  ...
  bot.on('text', async (ctx, next) => {
    ...
    console.log(
      `[invoice.handler.js] Recibido texto: "${texto}", estado: ${JSON.stringify(ctx.userState)}`
    );
    ...
    if (
      ...
      switch (ctx.userState.esperando) {
        ...
        case 'monto': {
          ...
          const { message, keyboard, parse_mode } = invoiceSummaryView(
            ctx.userState,
            ctx.userState.transactionId
          );
          ctx.reply(message, { parse_mode, ...keyboard });
          break;
        }
