...
async function createFacturapiOrganization(name) {
  ...
  try {
    const response = await axios.post(
      'https://www.facturapi.io/v2/organizations',
      { name },
      {
        headers: {
...
export function registerOnboardingHandler(bot) {
  ...
  bot.on('text', async (ctx, next) => {
    ...
    if (ctx.userState.esperando === RegistrationState.CREATE_ORGANIZATION) {
      ...
      if (text.length < 3) {
        return ctx.reply(
          'El nombre de la organización debe tener al menos 3 caracteres. Inténtalo de nuevo:'
        );
      }
      ...
      try {
        ...
        console.error(
          'Error al crear organización en FacturAPI:',
        ...
        await ctx.reply(
          ...
          Markup.inlineKeyboard([[Markup.button.callback('↩️ Volver', 'back_to_start')]])
        );
      }
    ...
    switch (ctx.userState.esperando) {
      case RegistrationState.BUSINESS_NAME:
        ...
        if (text.length < 3) {
          await ctx.reply(
            'El nombre de la empresa debe tener al menos 3 caracteres. Inténtalo de nuevo:'
          );
        } else {
      ...
      case RegistrationState.RFC:
        ...
        try {
          ...
          console.log(
            `RFC registrado: "${text.toUpperCase()}". Nuevo estado: ${ctx.userState.esperando}`
          );
          ...
          await ctx.reply(
            '❌ Ocurrió un error al verificar el RFC. Por favor, intenta nuevamente.'
          );
        }
        break;
      ...
      case RegistrationState.CONTACT_NAME:
        ...
        console.log(
          `Nombre de contacto registrado: "${text}". Nuevo estado: ${ctx.userState.esperando}`
        );
      ...
      case RegistrationState.NAME:
        ...
        console.log(
          `Nombre comercial registrado: "${text}". Nuevo estado: ${ctx.userState.esperando}`
        );
      ...
      case RegistrationState.ADDRESS_NEIGHBORHOOD:
        ...
        await ctx.reply('✅ Colonia registrada correctamente.\n\n' + 'Ahora, ingresa la ciudad:');
        break;
      ...
      case RegistrationState.ADDRESS_MUNICIPALITY:
        ...
        await ctx.reply('✅ Municipio registrado correctamente.\n\n' + 'Ahora, ingresa el estado:');
        break;
  ...
  bot.action('confirm_registration', async (ctx) => {
    ...
    try {
      ...
      try {
        ...
        if (!FACTURAPI_USER_KEY) {
          console.error(
            'Error: FACTURAPI_USER_KEY no está configurada en las variables de entorno'
          );
      ...
      try {
        ...
        await ctx.reply(
          '⚠️ Hubo un problema al configurar los datos fiscales. Podrás configurarlos más tarde desde el menú.'
        );
      }
      ...
      try {
        ...
        if (plans && plans.length > 0) {
          ...
          console.log(
            `Suscripción de prueba creada con ID: ${subscription.id}, vence: ${subscription.trialEndsAt}`
          );
      ...
      try {
        ...
        const clientSetupResults = await CustomerSetupService.setupPredefinedCustomers(
          tenant.id,
          true
        );
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([[Markup.button.callback('↩️ Volver al inicio', 'menu_principal')]])
      );
    ...
    try {
      ...
      if (tenant && tenant.facturapiApiKey) {
        ...
        if (isLiveKey) {
          ...
          await ctx.reply(
            '✅ Verificación completada: Tienes API key de producción configurada correctamente.'
          );
        } else {
          ...
          await ctx.reply(
            '✅ Verificación completada: Tienes API key de prueba configurada correctamente.'
          );
        }
      } else {
        await ctx.reply(
