  ...
  bot.action('menu_chubb', async (ctx) => {
    ...
    try {
      ...
      if (!chubbClient) {
        ...
        await ctx.reply(
          '⚠️ No se encontró el cliente CHUBB. Intentando configurar clientes predefinidos...'
        );
        ...
        try {
          ...
          if (!chubbClientAfterSetup) {
            return ctx.reply(
              '❌ Error: No se pudo encontrar o configurar el cliente CHUBB. Por favor, contacta al administrador.'
            );
          }
          ...
          console.log(
            `Cliente CHUBB configurado y encontrado: ${chubbClientAfterSetup.legalName} (ID: ${chubbClientAfterSetup.facturapiCustomerId})`
          );
        } catch (setupError) {
          console.error('Error al configurar clientes predefinidos:', setupError);
          return ctx.reply(
            '❌ Error: No se pudo configurar el cliente CHUBB. Por favor, contacta al administrador.'
          );
        }
      } else {
        ...
        console.log(
          `Cliente CHUBB encontrado: ${chubbClient.legalName} (ID: ${chubbClient.facturapiCustomerId})`
        );
      }
      ...
      await ctx.reply(
        'Por favor, sube el archivo Excel con los datos de CHUBB para generar las facturas.'
      );
    } catch (error) {
  ...
  bot.action('chubb_confirmar', async (ctx) => {
    ...
    try {
      ...
      if (facturas.length > 0) {
        ...
        facturas.forEach((factura, index) => {
          ...
          botones.push([
            Markup.button.callback(
              `📄 PDF ${factura.series}-${factura.folio_number}`,
              `pdf_${factura.id}_${factura.folio_number}`
            ),
            Markup.button.callback(
              `🔠 XML ${factura.series}-${factura.folio_number}`,
        ...
        await ctx.reply(
          '⚠️ No se generó ninguna factura. Por favor, verifica los datos del archivo Excel.'
        );
      }
...
async function procesarArchivoChubb(ctx, filePath, progressMessageId = null) {
  try {
    ...
    if (!columnMappings) {
      ...
      await ctx.reply(
        '❌ El archivo Excel no tiene todas las columnas requeridas. Se necesitan columnas para: Número de Caso, Servicio, Monto y datos de retención.'
      );
    ...
    await updateProgressMessage(
      ...
      5,
      6,
      'Clasificando servicios',
      'Agrupando por tipo de servicio...'
    );
    ...
    if (totalRegistros === 0) {
      await ctx.reply(
        '❌ No se encontraron datos válidos para generar facturas después de aplicar los criterios de clasificación.'
      );
    ...
    await updateProgressMessage(
      ...
      `${totalRegistros} registros listos para facturar`
    );
...
function clasificarDatos(data, columnMappings) {
  ...
  data.forEach((row, index) => {
    try {
      ...
      const servicio = String(row[columnMappings.servicio] || '')
        .trim()
        .toUpperCase();
  ...
  // Log de resumen
  console.log(
    `Clasificación completada: ${grupos.gruaConRetencion.length} con retención, ${grupos.gruaSinRetencion.length} sin retención, ${grupos.otrosServicios.length} otros servicios`
  );
 ...
 */
async function generarFacturaParaGrupo(
  items,
  claveSAT,
  conRetencion,
  ctx,
  ...
  if (montoTotal <= 0) {
    await ctx.reply(
      `⚠️ No se generó factura para clave SAT ${claveSAT} porque el monto total es 0.`
    );
    return null;
  }
  ...
  try {
    await ctx.reply(
      `⏳ Generando factura para: ${claveSAT} (${conRetencion ? 'Con' : 'Sin'} retención)...`
    );
    ...
    console.log(
      'Factura creada en FacturAPI, folio asignado automáticamente:',
