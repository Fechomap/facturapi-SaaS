...
export function registerProductionSetupHandler(bot) {
    ...
    try {
      ...
      try {
        ...
        if (
          orgInfo &&
          orgInfo.legal &&
          orgInfo.legal.certificate &&
          orgInfo.legal.certificate.expires_at
        ) {
          return ctx.reply(
  ...
  bot.on('document', async (ctx, next) => {
    ...
    if (!isCertFile || !ctx.userState?.productionSetup) {
      console.log(
        'No es archivo de certificado o no hay config productiva, pasando al siguiente handler'
      );
    ...
    try {
      ...
      if (setup.state === SetupState.AWAITING_CER && document.file_name.endsWith('.cer')) {
        ...
        console.log(
          '=========== FIN DIAGNÓSTICO DE DOCUMENTO PRODUCCIÓN (PROCESADO CER) ==========='
        );
        ...
        console.log(
          '=========== FIN DIAGNÓSTICO DE DOCUMENTO PRODUCCIÓN (PROCESADO KEY) ==========='
        );
        return; // No continuar con los siguientes handlers
      }
      ...
      console.log(
        '=========== FIN DIAGNÓSTICO DE DOCUMENTO PRODUCCIÓN (ESTADO INCORRECTO) ==========='
      );
  ...
  bot.action(/^complete_setup_(.+)$/, async (ctx) => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `🧹 Cache de FacturAPI limpiado después de actualizar API key para tenant ${tenantId}`
        );
      } catch (cacheError) {
      ...
      try {
        ...
        console.log(
          `Se eliminaron ${deletedInvoices.count} facturas de prueba para el tenant ${tenantId}`
        );
