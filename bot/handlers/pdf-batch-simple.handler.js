...
export function handleBatchGenerateInvoices(bot) {
  bot.action('batch_generate_invoices', async (ctx) => {
    ...
    if (!batchData || !batchData.results || batchData.results.length === 0) {
      return ctx.reply('❌ No hay datos de análisis disponibles. Por favor, procese los PDFs nuevamente.');
    }
  ...
  try {
    ...
    for (let i = 0; i < successfulInvoices.length; i++) {
      ...
      try {
        ...
        const folio = inv.invoice.folio_number || inv.invoice.folio || inv.invoice.folioNumber || 'SIN_FOLIO';
        const series = inv.invoice.series || 'F';
        ...
        console.error(`❌ Error descargando ${type} para factura ${inv.invoice.series}${inv.invoice.folio_number}:`, error.message);
        if (error.response) {
    ...
    console.log(`📊 Total de archivos agregados al ZIP: ${filesAdded} de ${successfulInvoices.length}`);
