...
async function validateOnboarding() {
  ...
  try {
    ...
    console.log(`- Pasos completados: ${initialProgress.completedSteps.length} de ${initialProgress.completedSteps.length + initialProgress.pendingSteps.length}`);
    ...
    if (response.toLowerCase() === 's') {
      ...
      if (initialProgress.pendingSteps.length === 0) {
        ...
        const stepIndex = await new Promise(resolve => {
          readline.question(`\nSelecciona un paso para completar (1-${initialProgress.pendingSteps.length}): `, resolve);
        });
