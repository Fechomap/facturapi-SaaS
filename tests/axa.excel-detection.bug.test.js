...
describe('AXA - BUG DETECCIÓN EXCEL', () => {
  ...
  test('BUG REPRODUCIDO: Primer Excel no detectado, segundo sí', () => {
    ...
    console.log(`❌ PRIMER INTENTO: esperando="${mockCtx.userState.esperando}", detectado=${excelDetectado1}`);
    ...
    console.log(`✅ SEGUNDO INTENTO: esperando="${mockCtx.userState.esperando}", detectado=${excelDetectado2}`);
  ...
  test('SOLUCIÓN PROPUESTA: Verificar estado en múltiples fuentes', () => {
    ...
    function debeDetectarExcel(ctx) {
      ...
      if (tempData && tempData.esperandoExcel) {
        ...
        if (tiempoEspera < 300000) { // 5 minutos max
