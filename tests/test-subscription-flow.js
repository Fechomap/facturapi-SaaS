...
async function testSubscriptionFlow() {
  ...
  try {
    ...
    if (!plan) {
      ...
      testLogger.warn({ planId: plan.id }, 'El plan existente no tiene stripePriceId. Asignando uno mock para la prueba.');
      plan.stripePriceId = 'price_mock_test_existing'; 
    }
    
    testLogger.info({ planId: plan.id, planName: plan.name, stripePriceId: plan.stripePriceId }, 'Plan de suscripción listo');
    ...
    if (updatedSubscription.status !== 'payment_pending') {
      throw new Error(`La suscripción no cambió a payment_pending, estado actual: ${updatedSubscription.status}`);
    }
    ...
    if (!refreshedTenant || !refreshedTenant.stripeCustomerId) {
      throw new Error(`No se pudo obtener el stripeCustomerId para el tenant ${tenant.id} después del job.`);
    }
    ...
    if (finalSubscription.status !== 'active') {
      throw new Error(`La suscripción no se activó correctamente, estado actual: ${finalSubscription.status}`);
    }
