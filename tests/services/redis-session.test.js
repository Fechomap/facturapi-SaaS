...
describe('Redis Session Service', () => {
  ...
  describe('KEYS vs SCAN Performance', () => {
    it('should use SCAN instead of KEYS for getting all sessions', async () => {
      ...
      if (redisSessionService.redis && redisSessionService.isConnected) {
        ...
        for (let i = 0; i < 1000; i++) {
          await redisSessionService.setSession(`test-${i}`, {
            userId: i,
            data: `test-data-${i}`
          }, 3600);
        }
    ...
    it('should implement SCAN alternative for production safety', async () => {
      ...
      // Add test sessions
      for (let i = 0; i < 100; i++) {
        await redisSessionService.setSession(`scan-test-${i}`, {
          userId: i
        }, 3600);
      }
    ...
    it('should handle concurrent session reads/writes', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        if (i % 2 === 0) {
          // Write operation
          operations.push(
            redisSessionService.setSession(sessionId, {
              counter: i,
              timestamp: Date.now()
            }, 3600)
          );
          ...
          // Read operation
          operations.push(
            redisSessionService.getSession(sessionId)
          );
        }
      }
