...
describe('BUSCAR CUELLOS DE BOTELLA: AXA vs CHUBB', () => {
  ...
  test('CUELLO DE BOTELLA 2: Precálculo lento durante Excel (como teníamos en CHUBB)', () => {
    ...
    const datosGrandes = Array(100).fill(null).map((_, i) => ({
    ...
    console.log(`📊 DATOS: ${datosGrandes.length} registros, ${JSON.stringify(datosGrandes).length} bytes`);
    ...
    console.log(`📊 RESULTADO: ${Object.keys(grupos).length} grupos, ${itemsParaFacturAPI.length} items`);
