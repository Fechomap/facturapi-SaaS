...
describe('INVESTIGACIÓN: Seguridad de limpiar pdfAnalysis', () => {
  ...
  test('ESCENARIO 1: Usuario en flujo PDF → cambia a AXA', () => {
    ...
    console.log(
      `  📦 userState.pdfAnalysis: ${JSON.stringify(mockCtx.userState.pdfAnalysis).length} bytes`
    );
    console.log(
      `  💾 session.pdfAnalysis: ${JSON.stringify(mockCtx.session.pdfAnalysis).length} bytes`
    );
  ...
  test('ESCENARIO 2: Usuario vuelve a PDF después de AXA', () => {
    ...
    console.log(`  📦 userState.pdfAnalysis: ${mockCtx.userState.pdfAnalysis || 'LIMPIO'}`);
    console.log(
      `  💾 session.pdfAnalysis: ${mockCtx.session.pdfAnalysis ? 'EXISTE' : 'NO EXISTE'}`
    );
  ...
  test('IMPLEMENTACIÓN: Función de limpieza segura', () => {
    ...
    testScenarios.forEach((scenario) => {
      ...
      console.log(
        `  📦 userState limpiado: ${result.userStateCleanup}, existe: ${userStateExists}`
      );
