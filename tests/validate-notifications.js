...
async function validateNotifications() {
  ...
  try {
    ...
    const telegramId = await new Promise((resolve) => {
      readline.question(
        'Ingresa tu ID de Telegram para recibir la notificación de prueba: ',
        resolve
      );
    });
    ...
    const result = await NotificationService.sendTelegramNotification(telegramId, testMessage, {
      parse_mode: 'Markdown',
    });
