...
export function registerUserManagementCommands(bot) {
    ...
    ctx.reply(
      ...
      { 
        ...
        ...Markup.inlineKeyboard([
          [Markup.button.callback('❌ Cancelar', 'cancel_invite')]
        ])
      }
    );
  });
  ...
  bot.action('manage_users', checkPermission('user:manage'), async (ctx) => {
    ...
    try {
      ...
      ctx.reply(
        '👥 *Selecciona usuario para gestionar:*',
        {
          parse_mode: 'Markdown',
  ...
  bot.on('text', async (ctx, next) => {
    if (ctx.userState?.esperando === 'invite_telegram_id') {
      ...
      if (!/^\d+$/.test(telegramId)) {
        return ctx.reply(
          '❌ ID inválido. Debe ser solo números.\n\n' +
          'Ejemplo: `123456789`',
          { parse_mode: 'Markdown' }
        );
      }
  ...
  bot.action(/manage_user_(\d+)/, checkPermission('user:manage'), async (ctx) => {
    ...
    try {
      ...
      if (user.isAuthorized) {
        keyboard.push([Markup.button.callback('⏸️ Desautorizar', `unauthorize_${targetTelegramId}`)]);
      } else {
      ...
      // Remover (peligroso)
      keyboard.push([Markup.button.callback('🗑️ Remover usuario', `remove_user_${targetTelegramId}`)]);
