...
const logger = createModuleLogger('report-commands');
...
export function registerReportCommands(bot: Bot): void {
  ...
  bot.command('reporte_facturas', async (ctx: BotContext) => {
    if (!ctx.hasTenant()) {
      return ctx.reply(
  ...
  bot.action(/reporte_mes_(\d+)_(\d+)/, async (ctx: BotContext) => {
    ...
    try {
      ...
      const buttons = [];
      buttons.push([
        Markup.button.callback('⬅️ Mes anterior', `reporte_mes_${prevYear}_${prevMonth}`),
      ]);

      if (!isFutureMonth) {
        buttons.push([
          Markup.button.callback('➡️ Mes siguiente', `reporte_mes_${nextYear}_${nextMonth}`),
        ]);
      }
  ...
  bot.command('reporte_suscripcion', async (ctx: BotContext) => {
    if (!ctx.hasTenant()) {
      return ctx.reply('Para generar un reporte, primero debes registrar tu empresa.');
    }
  ...
  bot.command('admin_enviar_reportes', async (ctx: BotContext) => {
    ...
    if (!ADMIN_CHAT_IDS.includes(BigInt(ctx.from?.id || 0))) {
      ...
      for (let i = 0; i < tenants.length; i += BATCH_SIZE) {
        ...
        const batchResults = await Promise.allSettled(
          batch.map(async (tenant) => {
            try {
              return await ReportsService.sendReportByTelegram(tenant.id, 'monthly_invoice', {
          ...
          if (result.status === 'fulfilled' && result.value.success) {
            ...
            const errorMsg =
              result.status === 'rejected'
                ...
                : (result.value as ReportResult).error || 'Error desconocido';
            results.details.push({
              tenantId: tenant.id,
              businessName: tenant.businessName,
              success: false,
        ...
        await ctx.reply(`✅ Progreso: ${i + batch.length}/${tenants.length} tenants procesados`);
      }
  ...
  logger.info('Comandos de reportes registrados correctamente');
}
