...
async function setupPredefinedClients(tenantId, forceAll = false) {
  try {
    ...
    if (!tenant.facturapiOrganizationId) {
      throw new Error(
        `El tenant ${tenant.businessName} no tiene una organización de FacturAPI configurada`
      );
    }
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            console.error(
              `Error al reintentar con RFC genérico para ${clientData.legal_name}:`,
              retryError.message
            );
  ...
  try {
    ...
    if (localClients.length > 0) {
      ...
      try {
        ...
        const facturapiClient = await facturapi.customers.retrieve(
          localClients[0].facturapiCustomerId
        );
        console.log(
          `✅ Verificación exitosa: Cliente ${facturapiClient.legal_name} existe en FacturAPI`
        );
        ...
        return {
          success: false,
          error: 'Los clientes existen en la base de datos local pero no en FacturAPI',
        };
      }
    }
