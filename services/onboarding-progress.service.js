  ...
  static async updateProgress(tenantId, step, metadata = {}) {
    try {
      ...
      if (!stepExists) {
        ...
        if (allRequiredCompleted && !completedSteps.includes(OnboardingSteps.ONBOARDING_COMPLETED)) {
      ...
      return {
        ...
        completedSteps,
        pendingSteps,
        progress: Math.round((completedSteps.filter(s => REQUIRED_STEPS.includes(s)).length / REQUIRED_STEPS.length) * 100),
