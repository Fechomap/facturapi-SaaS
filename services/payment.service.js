...
async function savePayment(
  ...
  paymentLogger.info(
    { tenantId, subscriptionId, stripePaymentId, status },
    'Guardando registro de pago'
  );
  ...
  paymentLogger.info(
    {
      ...
      calculatedEndDate: nextBillingDate.toISOString(),
    },
    'Calculando fecha de próximo cobro'
  );
...
async function handleWebhookEvent(payload, signature, webhookSecret, secretKey) {
  ...
  try {
    ...
    paymentLogger.info(
      ...
      'Evento de Stripe recibido y verificado'
    );
  } catch (err) {
  ...
  try {
    ...
    paymentLogger.error(
      { error: processingError.message, eventType: event.type, eventId: event.id },
      'Error al procesar evento de webhook'
    );
...
async function handleCheckoutSessionCompleted(session) {
  ...
  if (session.payment_status !== 'paid') {
    paymentLogger.warn(
      ...
      'Checkout session no tuvo pago exitoso, ignorando.'
    );
  ...
  if (!stripeCustomerId) {
    paymentLogger.error(
      ...
      'No se encontró Stripe Customer ID en la sesión de checkout.'
    );
  ...
  if (!tenant) {
    paymentLogger.error(
      ...
      'No se encontró un Tenant para el Stripe Customer ID.'
    );
  ...
  if (!dbSubscription) {
    paymentLogger.warn(
      ...
      'No se encontró una suscripción en payment_pending para este tenant. ¿Quizás ya se procesó o es un pago inesperado?'
    );
    ...
    if (activeSub && new Date().getTime() - activeSub.updatedAt.getTime() < 5 * 60 * 1000) {
      // 5 minutos margen
      paymentLogger.info(
        'Se encontró una suscripción activa recientemente actualizada, asumiendo que este evento ya fue procesado.'
      );
    ...
    paymentLogger.error(
      {
        sessionId: session.id,
        expectedAmount,
        gotAmount: session.amount_total,
        expectedCurrency,
        gotCurrency: session.currency,
      },
  ...
  paymentLogger.info(
    ...
    'Suscripción actualizada a activa con nuevas fechas.'
  );
  ...
  if (paymentIntentId) {
    try {
      ...
      paymentLogger.warn(
        { error: piError.message, paymentIntentId },
        'No se pudo recuperar el Payment Intent, usando datos limitados.'
      );
    }
  } else {
  ...
  try {
    ...
    paymentLogger.warn(
      { error: notifyError.message, tenantId: tenant.id },
      'Error al enviar notificación de pago exitoso.'
    );
  }
...
async function handleSubscriptionCreated(subscription) {
  paymentLogger.info(
    ...
    'Procesando customer.subscription.created'
  );
  ...
  if (!tenantId) {
    paymentLogger.error(
      ...
      'No se pudo determinar el TenantID para la suscripción creada.'
    );
    return {
      success: false,
      error: 'TenantID no encontrado',
  ...
  if (dbSubscription) {
    ...
    paymentLogger.info(
      ...
      'Actualizando suscripción existente en DB.'
    );
    await prisma.tenantSubscription.update({
      where: { id: dbSubscription.id },
    ...
    if (!stripePriceId) {
      paymentLogger.error(
        { subscriptionId: subscription.id },
        'No se pudo determinar el ID de precio de Stripe en la suscripción creada.'
      );
      return {
        success: false,
        error: 'Price ID no encontrado',
        eventType: 'customer.subscription.created',
      };
    }
    ...
    if (!plan) {
      ...
      return {
        success: false,
        error: `Plan no encontrado para Price ID ${stripePriceId}`,
        eventType: 'customer.subscription.created',
      };
    }
    ...
    paymentLogger.info(
      ...
      'Nueva suscripción creada en DB.'
    );
...
async function handleSubscriptionUpdated(subscription) {
  paymentLogger.info(
    ...
    'Procesando customer.subscription.updated'
  );
  const dbSubscription = await prisma.tenantSubscription.findFirst({
    where: {
      stripeSubscriptionId: subscription.id,
  ...
  if (!dbSubscription) {
    paymentLogger.error(
      `No se encontró la suscripción con ID ${subscription.id} en la base de datos`
    );
  ...
  paymentLogger.info(
    { dbSubscriptionId: dbSubscription.id, oldStatus: dbSubscription.status, newStatus },
    'Actualizando estado de suscripción en DB.'
  );
  ...
  if (newStatus === 'suspended' && dbSubscription.status !== 'suspended') {
    ...
    paymentLogger.info(
      ...
      'Tenant marcado como inactivo debido a suscripción suspendida.'
    );
    // Considerar enviar notificación de suspensión
  } else if (
    newStatus === 'active' &&
    (dbSubscription.status === 'suspended' || !dbSubscription.tenant.isActive)
  ) {
...
async function handleSubscriptionDeleted(subscription) {
  paymentLogger.info(
    { subscriptionId: subscription.id },
    'Procesando customer.subscription.deleted'
  );
  const dbSubscription = await prisma.tenantSubscription.findFirst({
    where: {
      stripeSubscriptionId: subscription.id,
  ...
  paymentLogger.info(
    { dbSubscriptionId: dbSubscription.id },
    'Actualizando estado a cancelled en DB.'
  );
  await prisma.tenantSubscription.update({
    where: { id: dbSubscription.id },
    data: {
...
async function handleInvoicePaymentSucceeded(invoice) {
  paymentLogger.info(
    { invoiceId: invoice.id, subscriptionId: invoice.subscription, customerId: invoice.customer },
    'Procesando invoice.payment_succeeded'
  );
  ...
  if (!dbSubscription) {
    paymentLogger.error(
      `No se encontró la suscripción con ID ${invoice.subscription} en la base de datos para la factura ${invoice.id}`
    );
  ...
  const payment = await savePayment(
    ...
    invoice.payment_settings?.payment_method_details?.card?.brand ||
      invoice.payment_settings?.payment_method_type ||
      'card', // Método de pago
    'succeeded'
  );
  ...
  if (
    ...
    paymentLogger.info(
      { startDate, endDate },
      'Usando fechas de período de la factura para actualización.'
    );
  } else {
    paymentLogger.info(
      { startDate, endDate },
      'Manteniendo fechas de período existentes o de evento de suscripción.'
    );
  }
  ...
  paymentLogger.info(
    { dbSubscriptionId: dbSubscription.id, newStatus },
    'Estado y fechas de suscripción (re)confirmados.'
  );
  ...
  if (needsReactivation) {
    ...
    paymentLogger.info(
      { tenantId: dbSubscription.tenantId },
      'Tenant reactivado por pago de factura.'
    );
...
async function handleInvoicePaymentFailed(invoice) {
  paymentLogger.warn(
    { invoiceId: invoice.id, subscriptionId: invoice.subscription, customerId: invoice.customer },
    'Procesando invoice.payment_failed'
  );
  // Verificar si esta factura está asociada a una suscripción
  if (!invoice.subscription) {
  ...
  if (!dbSubscription) {
    paymentLogger.error(
      `No se encontró la suscripción con ID ${invoice.subscription} en la base de datos para la factura fallida ${invoice.id}`
    );
    return {
      success: false,
      error: 'Suscripción no encontrada',
  ...
  paymentLogger.info(
    { dbSubscriptionId: dbSubscription.id, newStatus },
    'Estado de suscripción actualizado por fallo de pago.'
  );
  ...
  if (newStatus === 'suspended') {
    ...
    paymentLogger.info(
      { tenantId: dbSubscription.tenantId },
      'Tenant marcado como inactivo por fallo de pago final.'
    );
    // Enviar notificación de suspensión
    try {
      const adminUsers = await prisma.tenantUser.findMany({
        where: { tenantId: dbSubscription.tenantId, role: 'admin' },
        select: { telegramId: true },
      });
      const failMessage =
        `❌ *Pago Fallido y Suspensión*\n\n` +
      ...
      paymentLogger.warn({ error: notifyError.message }, 'Error al notificar suspensión.');
    }
  } else {
    // Si es 'payment_pending'
    ...
    try {
      ...
      const failMessage =
        `⚠️ *Fallo en el Pago de Renovación*\n\n` +
      ...
      paymentLogger.warn({ error: notifyError.message }, 'Error al notificar fallo de pago.');
    }
  }
...
function mapStripeStatusToDbStatus(stripeStatus) {
  ...
  switch (stripeStatus) {
    ...
    default:
      paymentLogger.warn(
        `Estado de Stripe no mapeado encontrado: ${stripeStatus}, usando 'payment_pending' por defecto.`
      );
